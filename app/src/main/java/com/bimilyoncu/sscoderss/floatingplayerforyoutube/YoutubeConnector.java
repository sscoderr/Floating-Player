...
public class YoutubeConnector {
    ...
    public static  final String[] myApiKeys={
            "AIzaSyDzYXfEUkLxWCO4Ah6MM_5iF8yjwJDAbVA",
    ...
    Token tk=new Token();
    TokenForChannel tkChannel=new TokenForChannel();
    TokenForSimilarVideos tkSimilarVideos =new TokenForSimilarVideos();
    List<VideoItem> items=new ArrayList<>();
    public List<VideoItem> search(String keywords,boolean first,boolean isChannel,boolean isSimilarVideos){
        if(!isChannel)
            query.setQ(keywords);
        try{
            if((!isChannel&&(tk.getNextToken()!=null||first==true))||(isChannel&&(tkChannel.getNextToken()!=null||first==true))||isSimilarVideos) {
                if (!first&&isSimilarVideos)
                    query.setPageToken(tkSimilarVideos.getNextToken());
                else if (!first&&!isChannel)
                    query.setPageToken(tk.getNextToken());
                else if(!first&&isChannel)
                    query.setPageToken(tkChannel.getNextToken());
                Log.e("Token:"+tk.getNextToken(),"sfd");
                ...
                if (isSimilarVideos)
                    tkSimilarVideos.setNextToken(response.getNextPageToken());
                else if(!isChannel)
                    tk.setNextToken(response.getNextPageToken());
                else if(isChannel)
                    tkChannel.setNextToken(response.getNextPageToken());
                }
            return items;
        }catch(IOException e){
            return null;
